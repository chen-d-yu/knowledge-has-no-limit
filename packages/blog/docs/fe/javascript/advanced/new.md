...
但是在`JavaScript`中，并不存在上述的概念，但类却是面向对象的核心。 个人猜想，为了照顾从面向对象语言转过来的小伙伴，
`JavaScript`利用原型链等机制去模拟类的作用，实现面向对象的编程范式。

但构造函数在`JavaScript`中并不是一种语法，而是一种规范，一种约定俗成的规范。
构造函数通常以大驼峰开头，它的本质是一个普通的函数，当这个普通函数被new关键字所操作时，它就拥有了构造函数的意义。
...
这个时候**模板——构造函数**的优势就体现出来了。
虽然每位同学都有各自的`name、gender、hobby`数据，但是它们的值是不同的
我们就可以将这些不同值的属性，当做构造函数的参数传递进去，
而相同值的属性`age`，就固定写死。
