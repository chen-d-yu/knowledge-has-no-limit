...
但在`JavaScript`中，`this`的指向可不是一成不变的，它很灵活，灵活到会根据 **_当前执行环境_** 的变化而变化
...
它们在传递第一个参数——`this`指向时，都有一个共同的特点，如果第一个实参为非引用数据类型，那么`this`会绑定到该实参的 **_包装类_** 上，**_如果实参没有包装类，那么当前次的显式绑定会变成独立调用，也就是指向全局`this`_**
...
3. 绑定规则的优先级
   - `new`和`apply/call`存在互斥，不能一同使用
   - `new`绑定 > `bind` > `apply/call` > 隐式绑定 > 独立调用
