...
-[x] 优化 hero 部分，现在的字体颜色和 banner 的颜色不搭 -[x] 每日一言部分放在 footer 底部 -[x] 需要添加一个 loading 页面，去找一下合适的，暂时找了 [sy-my-life] 的作为代替，还需要继续找 -[x] X 优化 404 页面，暂缓，没找到合适的，失败，暂时搁置 -[ ] 优化 nav、sidebar，都用脚本的方式生成 -[ ] 收集多一些背景图片，每次进入到博客时，随机循环 -[ ] 将 hero 部分的 image 优化成 [一个酷炫的头像悬停效果](https://juejin.cn/post/7196747356796518460) -[ ] 一个给页面上锁的组件 -[ ] 在 html 上预览思维导图 -[ ] 一个导航页面？？？ -[ ] 给 actions 部分，修改成几个水滴按钮？？？ 11231
